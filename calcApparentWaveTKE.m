...
%dataLocation is a string specifying where key data is located
dataLocation = 'C:\Users\michael\Documents\ADCP\DEMOZONE\';
%burstIndexRange is a two-element array that specifies the first and last
%burst number for which we must calculated the expected pseudo-TKE
...
%intrinsic frequency.
load([dataLocation 'burstWaveModalPeriod.mat']);
...
%100 is needed.
load([dataLocation 'burstWaveHeights.mat']);
...
load([dataLocation 'burstDepthSurfBins.mat']);
load([dataLocation 'demozoneMeanVelCorrDirn.mat']);
...
burstMaxBin_noZeros = burstMaxBin;
burstMaxBin_noZeros(burstMaxBin_noZeros == 0) = size(velMag,1);
surfBinIndices = sub2ind(size(velDirn),burstMaxBin_noZeros,1:burstEndIndex);
surfCurrentSpeed = velMag(surfBinIndices)';
burstMeanDepth = burstMeanDepth';
...
load([dataLocation 'burstWaveHeadings.mat']), burstWaveHeadings = burstWaveHeadings';
...
%burst-wise depths.
minBinWithData = min(burstMaxBin_noZeros);
...
for burstCtr = burstStartIndex:burstEndIndex
    ...
    kWaveAmplitude(burstCtr) = (0.5*burstIntrinsWaveFreq(burstCtr)*burstWaveAmp(burstCtr)/sinh(burstWvno(burstCtr)*burstMeanDepth(burstCtr)))^2;
...
%simplified when plotting the whole-record data.
    sidelobeDepth = burstMeanDepth(burstCtr)*(1 - cos(paramStruc.beamAngle));
    depthFromSurf = -paramStruc.binVertSize*((minBinWithData - 1):-1:0) - sidelobeDepth;
    waveBuoyPseudoTKE(burstCtr,:) = kWaveAmplitude(burstCtr)*cosh(2*burstWvno(burstCtr)*(depthFromSurf + burstMeanDepth(burstCtr))); 
end
