...
filterHalfWidth = 0.0125;
filterDepth = 0.9;
testWSSTFiltered = testWSST;
for tCtr = 1:recordLength
    filterMask = ones(length(testFreqVec),1);
    waveFreq = waveFreqRidge(tCtr);
    filterMask(testFreqVec > (waveFreq - filterHalfWidth) & testFreqVec <= waveFreq) = ...
    ...
    filterMask(testFreqVec > waveFreq & testFreqVec < (waveFreq + filterHalfWidth)) = ...
        1 - filterDepth*((waveFreq + filterHalfWidth) - testFreqVec(testFreqVec > waveFreq & testFreqVec < (waveFreq + filterHalfWidth)))/filterHalfWidth;
    testWSSTFiltered(:,tCtr) = filterMask.*testWSST(:,tCtr);
end

figure
pcolor(testSampleTimes,testFreqVec,abs(testWSSTFiltered));
...
testTimeSeriesRecon = iwsst(testWSSTFiltered);
subplot(2,1,1)
plot(dT:dT:recordLengthSec,testTimeSeries(:,end)), hold on, plot(dT:dT:recordLengthSec,testTimeSeriesRecon)
...
for binCtr = 1:numBins
    ...
    for tCtr = 1:recordLength
        ...
        filterMask(testFreqVec > waveFreq & testFreqVec < (waveFreq + filterHalfWidth)) = ...
            1 - filterDepth*((waveFreq + filterHalfWidth) - testFreqVec(testFreqVec > waveFreq & testFreqVec < (waveFreq + filterHalfWidth)))/filterHalfWidth;
        testWSST(:,tCtr) = filterMask.*testWSST(:,tCtr);
    end
